...
11. **Interactive Audio Narrator & Auto-Scroll (`index.html`)**: You MUST include an interactive **"🔊 Narrate Architecture & Auto-Scroll"** feature in the web simulator (`index.html`) utilizing the browser's built-in Web Speech API (`window.speechSynthesis`). Add a master "🔊 Narrate All Walkthrough" button in the header and individual "🔊 Listen" buttons on each architectural explanation card (How, What & Why sections). When clicked, it must read out the text aloud while smoothly auto-scrolling (`element.scrollIntoView({ behavior: 'smooth', block: 'center' })`) and visually highlighting/glowing the active card being narrated!
...
- Include interactive features:
  ...
  - **Interactive Audio Narrator & Auto-Scroll**: Implement a Web Speech API (`window.speechSynthesis`) text-to-speech narrator with "🔊 Listen / Narrate" buttons on each step explanation card. When activated, the narrator reads the explanation aloud while smoothly auto-scrolling (`element.scrollIntoView({ behavior: 'smooth', block: 'center' })`) and applying a glowing active border to the card being read.
