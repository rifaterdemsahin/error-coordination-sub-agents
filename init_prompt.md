...
11. **Interactive Audio Narrator & Auto-Following Walkthrough (`index.html`)**: You MUST include an interactive **"🔊 Read Walkthrough & Follow Section"** feature in the web simulator (`index.html`) using the browser's built-in Web Speech API (`window.speechSynthesis`). Add a master "🔊 Read Walkthrough" button in the header and individual "🔊 Listen" buttons on each section/card. **CRITICAL REQUIREMENT**: When reading, the simulator MUST follow along in real-time — smoothly auto-scrolling (`element.scrollIntoView({ behavior: 'smooth', block: 'center' })`) to the exact related section being narrated and applying a glowing active outline/highlight to that section while its text is being spoken!
...
- Include interactive features:
  ...
  - **Interactive Audio Narrator & Auto-Following Walkthrough**: Implement a Web Speech API (`window.speechSynthesis`) text-to-speech narrator with a master "🔊 Read Walkthrough" button and individual "🔊 Listen" buttons on each step explanation card. **CRITICAL**: As each section or card is read aloud, the simulator MUST automatically follow along by smoothly auto-scrolling (`element.scrollIntoView({ behavior: 'smooth', block: 'center' })`) to that exact section and applying a glowing cyan active border/highlight to the section being read!
- 🎨 **Visual Explanation Image (Step 3), EMBEDDING & MODAL POPUP LIGHTBOX**: Generate a UI architecture illustration explaining **HOW** the browser simulation visualizes real-time metrics. **CRITICAL REQUIREMENT: Embed all generated step images (from Steps 0, 1, 2, and 3) directly into `index.html`** inside elegant glassmorphic cards with clear captions explaining the How, What, and Why for each step, heavily utilizing expressive emojis (🚀, ⚡, 💡, 🛡️, 🔍) for visual appeal! **Every embedded diagram and image MUST be clickable (`cursor: zoom-in`), opening in a responsive fullscreen modal popup lightbox with click-to-close and ESC key handling!**
